#!/usr/bin/env python3
"""Compile lean Hebrew lesson authoring data into Muhkam's baked exercise schema.

Input:  data/hebrew-src/*.json — each file: { "level": "<levelId>", "lessons": [...] }
        A lesson is either:
          regular:  { number, title, titleNative?, topicId?, items: [{he, en, translit?}] }
          reading:  { number, title, titleNative?, context?, paragraphs: [{he, en}],
                      questions: [{question, options, answerIndex}] }
        Optionally a file may also carry { "grammarTopics": { id: {title,pattern,explanation,fact} } }.
Output: data/courses-hebrew.json — full course document, same shape as data/courses.json.
"""
import json
import random
import re
...
SRC_DIR = ROOT / "data" / "hebrew-src"
OUT_PATH = ROOT / "data" / "courses-hebrew.json"

random.seed(20260720)  # deterministic distractor shuffling across runs

LEVEL_ORDER = ["a0", "a1", "a2", "b1", "b1plus", "b2", "b2plus", "c1"]
LEVEL_META = {
    "a0": dict(cefr="A0", label="First Steps", labelNative="צעדים ראשונים"),
    "a1": dict(cefr="A1", label="Foundations", labelNative="יסודות"),
    "a2": dict(cefr="A2", label="Building Blocks", labelNative="אבני בניין"),
    "b1": dict(cefr="B1", label="Everyday Fluency", labelNative="שטף יומיומי"),
    "b1plus": dict(cefr="B1+", label="Expanding Range", labelNative="הרחבת האופקים"),
    "b2": dict(cefr="B2", label="Complex Structures", labelNative="מבנים מורכבים"),
    "b2plus": dict(cefr="B2+", label="Precision & Nuance", labelNative="דיוק וניואנס"),
    "c1": dict(cefr="C1", label="Advanced", labelNative="מתקדמים"),
}
...
def compile_regular_lesson(level, lesson, en_pool_by_level, word_pool):
    ...
    for i, item in enumerate(items):
        ...
        if i % 3 == 0:
            ...
            exercises.append({"type": "listening-tap", "native": he, "answer": tokens})
    ...
    for start in range(0, len(items) - 3, 4):
        ...
        exercises.append({
            "type": "matching",
            "pairs": [{"native": c["he"], "en": c["en"]} for c in chunk],
        })

    out = {
        "id": f"he-{level}-{lesson['number']}",
...
def compile_reading_lesson(level, lesson):
    return {
        "id": f"he-{level}-{lesson['number']}-reading",
        "number": lesson["number"],
        "title": lesson["title"],
        ...
        "readingPassage": {
            ...
            "paragraphs": [{"native": p["he"], "en": p["en"]} for p in lesson["paragraphs"]],
        },
        "exercises": [
            {
                "type": "comprehension",
...
def main():
    ...
    course = {
        "id": "advanced-hebrew",
        "title": "Hebrew, Start to Advanced",
        "subtitle": "Modern Hebrew from absolute beginner through fluent, formal command — thousands of sentences of grammar and conversational practice",
        "dir": "rtl",
        "lang": "he",
        "languageName": "Hebrew",
        "fontNative": "'Frank Ruhl Libre', 'Noto Sans Hebrew', serif",
        "flag": "עברית",
        "heroEyebrow": "Modern Hebrew · A0 → C1",
        "heroNative": "מהיסודות ועד עברית שוטפת",
        "heroLedeSuffix": "Real sentences from lesson one, full niqqud to guide every word, gradually shedding the vowel marks as you climb.",
        "uiStrings": {"wordHoard": "אוצר מילים", "review": "חזרה", "revision": "תרגול"},
